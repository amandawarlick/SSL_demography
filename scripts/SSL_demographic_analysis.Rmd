...
describes the state *z* of individual *i* at occasion *t*, conditional on the individual’s state at the previous occasion, modeled as categorically distributed according to transition array $\Omega$, describing the probability of an individual transitioning to a new state based on its previous state and individual- and time-specific effects (Eq. \@ref(eq:ecolproc)). This transition array is decomposed into survival ($\phi_{i,t}$) and breeding probability ($\psi_{i,t}$) conditional on survival. 
...
\begin{equation}
\text{logit}(\gamma_\text{a,s}) = \mu_{\gamma_\text{a,s}} + \beta_{\gamma_\text{a,c}}\cdot x_{c,t} + \epsilon_{\gamma_\text{a,t}} (\#eq:gamma)
\end{equation}

where the mean intercept $\mu_{\gamma_\text{s,a}}$ for a given demographic rate $\gamma_\text{s,a}$ for each sex *s* and age *a* was estimated using a logit-transformed prior that was uniform U(0,1) on the probability scale. To increase parameter estimability and regulate model complexity, we applied penalized complexity priors (Simpson et al. 2017, van Erp et al. 2019) on the coefficients estimating the effect of environmental variables and random year effects. Using a penalized complexity prior shrinks the coefficient toward zero in the absence of strong support for a covariate effect. The effect of each individual characteristic (i.e., pup mass; $x_{i,t}$) and oceanographic variable *c* ($x_t$) on demographic rate $\gamma_\text{s,a}$ was assumed to be drawn from a unique Gaussian distribution as $\beta_{\gamma_\text{a,c}} \sim N(0, \sigma_{a,c})$, with standard deviations distributed according to an exponential distribution with a fixed shrinkage rate $\sigma_{c,a} \sim \text{Exp}(\lambda = 1)$ to apply moderately strong shrinkage. Similarly, random year effects were assumed to be drawn from a Gaussian distribution as $\epsilon_{\gamma_\text{a,t}} \sim N(0, \sigma_{\gamma{a}})$, with standard deviations $\sigma_{\gamma{a}}$ distributed according to an exponential distribution with a fixed shrinkage rate as described above. Age- and sex-specific intercepts were estimated for each demographic rate, but fixed effects of environmental conditions were shared across sexes for pups and additionally aggregated for age 1-2 individuals ("young" sea lions). Separate fixed effects were examined for the effect of pup mass for each sex. For survival, correlated random effects were assumed between sexes (except breeding females and adult males) and were also assumed for juvenile individuals ages 3-5 (i.e., year-specific deviations from the mean were modeled in common for these groups). For breeding probabilities, correlated random effects were assumed for all first-time breeding transitions for age 3-5 individuals.

To assist in the estimation of age-based survival for pups through age-5 individuals in the western portion of the range where sample sizes were smaller, mean survival rates $\mu_{\phi_\text{s,1:6}}$ were modeled as: 

\begin{equation}
\text{logit}(\gamma_{\text{s,1:6}}) = \mu_{\gamma_\text{s,1:6}} (\#eq:gammaWest)
\end{equation}

\begin{equation}
\mu_{\gamma_\text{s,1:6}} \sim \text{MVN}(0, \tau*Q) (\#eq:gammaWestMVNprior)
\end{equation}

by fitting an intrinsic autoregression (IAR(2)) using a multivariate normal distribution (Eq. \@ref(eq:gammaWest)-\@ref(eq:gammaWestMVNprior)) to smooth over the age classes with standard deviation $\sigma \sim \text{Exp}(\lambda = 1)$ used to derive $\tau = \frac{1}{\sigma^2}$ for use in the precision matrix, $Q$, which is given as data when fitting the model. The effects of environmental and individual-based covariates were estimated as above.

Age-based natality (the number of offspring produced per female ages 4-6, assuming singleton births) was calculated by taking the proportion of each female age class that had a pup at a given occasion according to the true *z* state. For the eastern portion of the range, age-based and overall natality were calculated beginning in the 7th study year to allow for more than one branded cohort to have reached breeding age. For the shorter study period in the western portion of the range, natality was calculated beginning in the 4th year when at least one branded cohort had reached breeding age.  
...
Models were fit using NIMBLE (NIMBLE Development Team 2019) within the R programming environment (R Core Development 2018) using 20,000-40,000 iterations and 10,000-20,000 burn-in depending on the model, a thinning rate of 1, and an adaptation rate of 10. We evaluated model convergence using visual inspection of chains and the Brooks-Gelman-Rubin statistic (Gelman & Rubin 1992; Brooks & Roberts 1998) $\hat{R}$ < 1.1. After fitting full models, we evaluated goodness-of-fit using Bayesian p-values, where we compared the number of observed versus predicted resightings of individuals by age at each occasion. The typical set of mark-recapture model assumptions applied in this study, where it was assumed that branding did not affect detection probability, that survival and the reproductive state of individuals were independent, there were no identification errors, mortality during the sampling season was negligible, and that there was no unmodeled heterogeneity in survival and detection probabilities.
