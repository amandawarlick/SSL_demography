...
by fitting an intrinsic autoregression (IAR(2)) using a multivariate normal distribution (Eq. \@ref(eq:gammaWest)-\@ref(eq:gammaWestMVNprior)) to smooth over the age classes 1:6 with standard deviation $\sigma \sim \text{Exp}(\lambda = 1)$ used to derive $\tau = \frac{1}{\sigma^2}$ for use in the precision matrix, $Q$, which is given as data when fitting the model. The effects of environmental and individual-based covariates were estimated as above.
...
Mean age-based natality (calculated from the 7th year onward) was low for age-4 individuals (`r round(mean(nat.vals[nat.vals$Region == 'Eastern' & nat.vals$age == '4', 'med']),2)`; `r round(mean(nat.vals[nat.vals$Region == 'Eastern' & nat.vals$age == '4', 'lower']),2)`-`r round(mean(nat.vals[nat.vals$Region == 'Eastern' & nat.vals$age == '4', 'upper']),2)`) and much higher for age-5 (`r round(mean(nat.vals[nat.vals$Region == 'Eastern' & nat.vals$age == '5', 'med']),2)`; `r round(mean(nat.vals[nat.vals$Region == 'Eastern' & nat.vals$age == '5', 'lower']),2)`-`r round(mean(nat.vals[nat.vals$Region == 'Eastern' & nat.vals$age == '5', 'upper']),2)`). Overall natality (proportion of breeding-age females with a pup each year) was calculated as approximately `r round(mean(nat.vals[nat.vals$Region == 'Eastern' & nat.vals$age == 'all' & nat.vals$year>10, 'med']),2)` (`r round(mean(nat.vals[nat.vals$Region == 'Eastern' & nat.vals$age == 'all' & nat.vals$year>10, 'lower']),2)`-`r round(mean(nat.vals[nat.vals$Region == 'Eastern' & nat.vals$age == 'all' & nat.vals$year>10, 'upper']),2)`) in the last few years of the study period. 
...
The effect of individual characteristics and environmental conditions ($\beta$) are reported on the logit scale, where values above zero indicate a positive correlation and values below zero a negative correlation. We report both the logit-scale value of $\beta_\text{c,a}$ and the probability that a given effect was above or below zero (p$(\beta\text{c,a} > 0)$). For individuals branded in the eastern portion of the range, pup mass had a positive effect on pup survival probability for both females ($\beta_\text{mass}$ = `r round(cov_dat[cov_dat$Region == 'east' & cov_dat$variable == 'b.massP','med'],2)`; `r round(cov_dat[cov_dat$Region == 'east' & cov_dat$variable == 'b.massP','lower'],2)`-`r round(cov_dat[cov_dat$Region == 'east' & cov_dat$variable == 'b.massP','upper'],2)`) and males ($\beta_\text{mass}$ = `r round(cov_dat[cov_dat$Region == 'east' & cov_dat$variable == 'b.massPM','med'],2)`; `r round(cov_dat[cov_dat$Region == 'east' & cov_dat$variable == 'b.massPM','lower'],2)`-`r round(cov_dat[cov_dat$Region == 'east' & cov_dat$variable == 'b.massPM','upper'],2)`), and likely female age-1-2 individuals as well (p$(\beta > 0)$ = `r prob_vals_env[prob_vals_env$Region == 'east' & prob_vals_env$Season == 'win' & prob_vals_env$var == 'b.mass1', 'prob']`) (`r figs('bmi_plot', display = 'cite')`). However, for individuals branded in the western portion of the range, pup mass had a negative effect on male pup survival ($\beta$ = `r round(cov_dat[cov_dat$Region == 'west' & cov_dat$variable == 'b.massPM','med'],2)`; `r round(cov_dat[cov_dat$Region == 'west' & cov_dat$variable == 'b.massPM','lower'],2)`-`r round(cov_dat[cov_dat$Region == 'west' & cov_dat$variable == 'b.massPM','upper'],2)`; p$(\beta < 0)$ = `r prob_vals_env[prob_vals_env$Region == 'west' & prob_vals_env$Season == 'win' & prob_vals_env$var == 'b.massPM', 'prob']`) and young age 1-2 survival ($\beta$ = `r round(cov_dat[cov_dat$Region == 'west' & cov_dat$variable == 'b.mass1M','med'],2)`; `r round(cov_dat[cov_dat$Region == 'west' & cov_dat$variable == 'b.mass1M','lower'],2)`-`r round(cov_dat[cov_dat$Region == 'west' & cov_dat$variable == 'b.mass1M','upper'],2)`; p$(\beta < 0)$ = `r prob_vals_env[prob_vals_env$Region == 'west' & prob_vals_env$Season == 'win' & prob_vals_env$var == 'b.mass1M', 'prob']`). Uncertainty in the estimates for the effect of pup mass on these various demographic rates is much greater for individuals branded in the western portion of the range. We did not detect an effect of pup mass on the probability of first-time breeding probability ($\psi_\text{3:5}$; `r figs('bmi_plot', display = 'cite')`). 
...
<!-- However, because the effects of ocean conditions are dynamic, vary over time and in three-dimensional space, and are complicated by potential lag effects of unknown duration, the design of this study precludes specifically identifying the underlying mechanisms driving observed correlations.  -->

<!-- One of the strengths of this study is its broad spatio-temporal scope, which likely smooths over the effects of cohort variability and therefore may be more representative of longer-term average demographic rates for the eastern portion of the range. -->

Differences in demographic rates across ages, sexes, and geographic divisions within the western DPS similarly offer the potential for improved ecological understanding that can inform management.
