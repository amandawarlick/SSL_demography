...
where an observation $y_{i,t}$ conditional on the true state $z_{i,t}$ is categorically distributed with probability array $\Theta$. Components of detection probability for individual *i* at time *t* include the probability of detection, $p_{i,t}$, and the probability of correctly ascertaining the presence of a pup for breeders, $\delta_{i,t}$. We accounted for potential variability in detection probability for individuals branded in the eastern portion of the range by allowing for random temporal variability, using penalized complexity priors, in addition to a fixed effect to account for markedly lower resight survey effort in three years during the study period (2006, 2017, 2018). We expected that the probability of correctly ascertaining whether a female had a pup would be a function of the number of times a female was seen in a season, and so we used the number of sightings per individual per year as a covariate for the multi-event classification probability parameter, $\delta_{i,t}$. We assumed that false positive identifications (non-breeders observed with pups) were not possible. Priors for parameters in the ecological state and observation process models were estimated using a logit-transformed prior of U(0,1) on the probability scale.

Variable and model selection -- Our first step in variable selection was to evaluate covariates using the Watanabe-Akaike information criterion (WAIC; Watanabe 2010). We built models with single environmental covariates and compared them to models without covariates (Supplemental information). Covariates without support were eliminated from further consideration. Models were evaluated separately for the eastern and western portions of the range of the western DPS, resulting in some differences in retained covariates between these two areas.

Once the final set of environmental covariates was determined, we used WAIC to compare the null model, an interannual random effects-only model (no environmental covariates), and a set of full models (interannual random effects, pup body mass, and season-specific environmental covariates). Environmental covariates were only included as fixed effects in the estimation of pup survival, survival of yearlings and age-2 individuals, and breeding probabilities for first-time and repeat breeding individuals, as we hypothesized that the survival of older individuals was likely to be relatively unaffected by environmental variability due to much larger energy storage capacity and foraging experience that enables them to diversify their diet or withstand poorer foraging conditions. Season-specific environmental covariates were examined together (i.e., all covariates were from the same season in each model run) due to the infeasibility of examining all possible combinations of the four seasonal values of each environmental covariate. We summarized the results of the full model according to the proportion of MCMC chain sample estimates for the environmental effects that was above versus below zero. 

Models were fit using NIMBLE (NIMBLE Development Team 2019) within the R programming environment (R Core Development 2018) using 20,000-40,000 iterations and 10,000-20,000 burn-in depending on the model, a thinning rate of 1, and an adaptation rate of 10. We evaluated model convergence using visual inspection of chains and the Brooks-Gelman-Rubin statistic (Gelman & Rubin 1992; Brooks & Roberts 1998) $R_\hat$ < 1.1. After fitting full models, we evaluated goodness-of-fit using Bayesian p-values, where we compared the number of observed versus predicted resightings of individuals by age at each occasion. The typical set of mark-recapture model assumptions applied in this study, where it was assumed that branding did not affect detection probability, that survival and the reproductive state of individuals were independent, there were no identification errors, mortality during the sampling season was negligible, and that there was no unmodeled heterogeneity in survival and detection probabilities.
...
Eastern portion of the range -- The probability of becoming a breeder was highest for age-5 individuals (i.e., giving birth for the first time at age 5; $\psi_4$) at `r round(post_sum_e['int.psi4','med'],2)` (`r round(post_sum_e['int.psi4','lower'],2)`-`r round(post_sum_e['int.psi4','upper'],2)`) followed by age-6 individuals at `r round(post_sum_e['int.psi5','med'],2)` (`r round(post_sum_e['int.psi5','lower'],2)`-`r round(post_sum_e['int.psi5','upper'],2)`) and much lower for age-4 individuals at `r round(post_sum_e['int.psi3','med'],2)` (`r round(post_sum_e['int.psi3','lower'],2)`-`r round(post_sum_e['int.psi3','upper'],2)`) (`r figs("psi_plot", display = "cite")`). The probability of breeding for females that had not given birth in the previous year ($\psi_\text{NB}$; age-5+ that had previously bred or age-7+ individuals that had never bred) was low (`r round(post_sum_e['int.psiNB','med'],2)`; `r round(post_sum_e['int.psiNB','lower'],2)`-`r round(post_sum_e['int.psiNB','upper'],2)`). The probability of breeders continuing to breed ($\psi_\text{B}$) was high (`r round(post_sum_e['int.psiB','med'],2)`; `r round(post_sum_e['int.psiB','lower'],2)`-`r round(post_sum_e['int.psiB','upper'],2)`). Temporal variance in breeding probabilities for first-time breeders was estimated with greater uncertainty ($\sigma_{\psi_\text{PB}}$ = `r round(sigma_vals['sigma.psiPB', 'med'],2)`) and fluctuated throughout the study period (`r figs("time_vary_plot", display = "cite")`b). Breeding probability given an individual was a breeder in the previous year remained relatively constant throughout the study period ($\sigma_{\psi_\text{B}}$ = `r round(sigma_vals['sigma.psiB', 'med'],2)`). The probability of breeding given an individual did not breed in the previous year remained relatively low over the study period with the exception of 2006-2007 but was estimated with a higher degree of uncertainty ($\sigma_{\psi_\text{NB}}$ = `r round(sigma_vals['sigma.psiNB', 'med'],2)`). 

Mean age-based natality (calculated from the 7th year onward once more than one cohort reached breeding age) was low for age-4 individuals (`r round(mean(nat.vals[nat.vals$Region == 'Eastern' & nat.vals$age == '4', 'med']),2)`; `r round(mean(nat.vals[nat.vals$Region == 'Eastern' & nat.vals$age == '4', 'lower']),2)`-`r round(mean(nat.vals[nat.vals$Region == 'Eastern' & nat.vals$age == '4', 'upper']),2)`) and much higher for age-5 (`r round(mean(nat.vals[nat.vals$Region == 'Eastern' & nat.vals$age == '5', 'med']),2)`; `r round(mean(nat.vals[nat.vals$Region == 'Eastern' & nat.vals$age == '5', 'lower']),2)`-`r round(mean(nat.vals[nat.vals$Region == 'Eastern' & nat.vals$age == '5', 'upper']),2)`) and age-6+ individuals (`r round(mean(nat.vals[nat.vals$Region == 'Eastern' & nat.vals$age == '6', 'med']),2)`; `r round(mean(nat.vals[nat.vals$Region == 'Eastern' & nat.vals$age == '6', 'lower']),2)`-`r round(mean(nat.vals[nat.vals$Region == 'Eastern' & nat.vals$age == '6', 'upper']),2)`) (`r figs("nat_plot", display = "cite")`). Overall natality (proportion of breeding-age females with a pup each year) was calculated as approximately `r round(mean(nat.vals[nat.vals$Region == 'Eastern' & nat.vals$age == 'all' & nat.vals$year>10, 'med']),2)` (`r round(mean(nat.vals[nat.vals$Region == 'Eastern' & nat.vals$age == 'all' & nat.vals$year>10, 'lower']),2)`-`r round(mean(nat.vals[nat.vals$Region == 'Eastern' & nat.vals$age == 'all' & nat.vals$year>10, 'upper']),2)`) in the last few years of the study period once the age-6+ plus group was theoretically dominated by mature experienced breeders. 

Western portion of the range -- Age-based breeding probabilities in the western portion of the range mirrored that in the eastern portion (higher probability for age-5 individuals and existing breeders), though the mean probability of breeding again ($\psi_\text{B}$) was slightly lower and credible intervals were much wider due to the smaller sample size (only two marked cohorts had reached breeding age by the end of the study, which covered fewer years compared to that in the eastern portion of the range; `r figs("psi_plot", display = "cite")`). Mean age-based natality calculated from the 4th year onward (once at least one cohort reached breeding age) was `r round(mean(nat.vals[nat.vals$Region == 'Western' & nat.vals$age == 4, 'med']),2)` (`r round(mean(nat.vals[nat.vals$Region == 'Western' & nat.vals$age == 4, 'lower']),2)`-`r round(mean(nat.vals[nat.vals$Region == 'Western' & nat.vals$age == 4, 'upper']),2)`) for age-4 individuals and `r round(mean(nat.vals[nat.vals$Region == 'Western' & nat.vals$age == 5, 'med']),2)` (`r round(mean(nat.vals[nat.vals$Region == 'Western' & nat.vals$age == 5, 'lower']),2)`-`r round(mean(nat.vals[nat.vals$Region == 'Western' & nat.vals$age == 5, 'upper']),2)`) for age-5 individuals. Natality for age-6+ in the western portion of the range only included a single cohort and are therefore not entirely comparable to the eastern portion of the range. Overall natality (proportion of females breeding) for the study period was approximately `r round(mean(nat.vals[nat.vals$Region == 'Western' & nat.vals$age == 'all' & nat.vals$year>16, 'med']),2)` (`r round(mean(nat.vals[nat.vals$Region == 'Western' & nat.vals$age == 'all' & nat.vals$year>16, 'lower']),2)`-`r round(mean(nat.vals[nat.vals$Region == 'Western' & nat.vals$age == 'all' & nat.vals$year>16, 'upper']),2)`; `r figs("nat_plot", display = "cite")`), though more years of data are needed. Time-varying demographic rates were not examined for individuals branded in the western portion of the range due to fewer branded individuals resighted over fewer occasions.

##### Detection 
Eastern portion of the range -- Detection probability increased with age for males and was estimated at `r round(post_sum_e['int.pBM','med'],2)` (`r round(post_sum_e['int.pBM','lower'],2)`-`r round(post_sum_e['int.pBM','upper'],2)`) for adults ($p_\text{AM}$ = `r figs("p_plot", display = "cite")`). Detection probability also generally increased with age for females, estimated at `r round(post_sum_e['int.pB','med'],2)` (`r round(post_sum_e['int.pB','lower'],2)`-`r round(post_sum_e['int.pB','upper'],2)`) for breeding adults ($p_\text{A}$). Detection probability was lower for age-5 pre-breeders ($p_\text{PB}$ = `r round(post_sum_e['int.p5','med'],2)`; `r round(post_sum_e['int.p5','lower'],2)`-`r round(post_sum_e['int.p5','upper'],2)`) and non-breeding individuals ($p_\text{NB}$ = `r round(post_sum_e['int.pNB','med'],2)`; `r round(post_sum_e['int.pNB','lower'],2)`-`r round(post_sum_e['int.pNB','upper'],2)`) (`r figs("p_plot", display = "cite")`). The temporal variance of detection probability over the study period was estimated with relatively low variance ($\sigma_\text{p}$ = `r round(sigma_vals['sigma.p', 'med'],2)`), including the offset ($\beta_p$ = `r round(post_sum_e['b.eff.2.', 'med'],2)` on the logit scale) to account for lower survey effort in years without dedicated field camps in 2006, 2016, and 2017 (`r figs("time_vary_plot", display = "cite")`c). The probability of correctly identifying females as having a pup ($delta$) increased with resighting frequency, ranging from `r round(post_sum_e['p.delB.1.','med'],2)` (`r round(post_sum_e['p.delB.1.','lower'],2)`-`r round(post_sum_e['p.delB.1.','upper'],2)`) for individuals resighted once or twice per year to `r round(post_sum_e['p.delB.3.','med'],2)` (`r round(post_sum_e['p.delB.3.','lower'],2)`-`r round(post_sum_e['p.delB.3.','upper'],2)`) for those resighted more than nine times. 
...
We used mark-resight data to estimate age- and sex-specific survival and natality and the effects of pup weight and oceanographic conditions for the western DPS of Steller sea lions in Alaska. These results represent the first demographic rate estimates for individuals branded in the western and central Aleutians where the population continues to decline (Sweeney et al. 2018). Survival and natality rates were comparable to other existing estimates for the species, with some notable differences between sexes, across regions, and across environmental conditions. These results provide insight into potential drivers of population dynamics and the relationship between demography and oceanographic conditions for this population. 

*Demographic comparisons*    
Researchers have hypothesized that the continued decline in counts at rookeries in the western portion of the range may be due to a combination of shifting age structure, lower demographic rates, or poor foraging conditions (York 1994, Sease et al. 2001, Gerber et al. 2001, Holmes & York 2007). With this study, we aimed to explore variation in age- and sex-specific survival as well as the explanatory variables driving that variability, thereby providing additional insights relevant to the multiple hypotheses posed to explain declines. Differences in demographic rates across ages, sexes, and geographic divisions within the western DPS similarly offer the potential for improved ecological understanding that can inform management.

Regional comparisons -- One consistent finding in many Steller sea lion studies is that survival estimates vary both between and within regions as well as over time. This spatio-temporal variation complicates direct comparison of results from different studies. However, in general, survival estimates in this study were similar to those estimated for this DPS in earlier time periods (York 1994, Pendleton et al. 2006, Fritz et al. 2014) and to those estimated for the eastern DPS (Hastings et al. 2011, 2021, Wright et al. 2017), though age- and sex-based differences were apparent (Dream Figure). York (1994) found that female survival in the Gulf of Alaska during the 1970s was approximately 0.78 for ages 1-3 (0.71-91 here), 0.90-0.93 for ages 4-5 (0.96-0.98 here), and 0.84-0.89 for age 6+ adults (0.93 here). During the 1990s, combined male and female estimates for the same region were a bit lower at 0.57-0.72 for ages 1-3 and 0.85 for age 4+ and lower still than estimates over the same period for rookeries in Southeast Alaska (Pendleton et al. 2006). Age-specific survival estimates within the Gulf of Alaska and eastern Aleutian Islands throughout the 2000s returned to levels similar to those estimated in the 1970s with the exception of yearling survival in the eastern Gulf of Alaska (Fritz et al. 2014). Ultimately, our estimates are similar to those reported for the eastern DPS in Alaska and for rookeries in Oregon and California (Wright et al. 2017, Hastings et al. 2021). 
...
```
`r figs('life_cyc', caption = 'Life cycle diagram.')`
...
`r tbls("pars_tab", caption = "Posterior mean and 95% credible intervals for all model parameters.")`
```{r pars table}
