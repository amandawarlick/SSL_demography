...
by fitting an intrinsic autoregression (IAR(2)) using a multivariate normal distribution (Eqs. \@ref(eq:gammaWest)-\@ref(eq:gammaWestMVNprior)) to smooth over the six age classes, with standard deviation $\sigma \sim \text{Exp}(\lambda = 1)$ used to derive $\tau = \frac{1}{\sigma^2}$ for use in the precision matrix, $Q$. 

Age-specific natality (*f*, the number of offspring produced per female ages 4-6, assuming only singleton births) was calculated by taking the proportion of each female age class that had a pup at a given occasion according to the true *z* state. For the eastern portion of the range, age-specific and overall natality (proportion of females breeding in a given year) were calculated beginning in the 7th study year to allow for more than one branded cohort to have reached breeding age. For the shorter study period in the western portion of the range, natality was calculated beginning in the 4th year when at least one branded cohort had reached breeding age.  
