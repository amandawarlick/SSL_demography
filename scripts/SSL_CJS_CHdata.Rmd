...
inits_raw <- read.csv(here::here('SSL_CJS', "Data", "initial_capture_data_02012021.csv"), 
...
res_raw <- read.csv(here::here('SSL_CJS', 'Data', 'resight_data_02012021.csv'), 
                     header = T, stringsAsFactors = F) 

resights <- res_raw %>%
  dplyr::select(-c(Sex, Beach, Platform, Time, datetime, Age)) %>% #sex from branding is more complete, but contains a few mistakes
  merge(inits_f, by = c('AnimalID'), all.y = T) %>% #take only females
...
ch_fem <- resights %>% 
  ...
  #duplicates largely bc sighted in dif regions -- I think could streamline with this instead 
...
num_resight <- resights %>%
  ...
  #scale can be adjusted; 9+ resights == 3, 3-8 resights == 2, 1-2 resights == 1, <1 == ND (4)
...
write.csv(num_resight_act, here::here('SSL_CJS', 'Data', 'resights_actual.csv'), row.names = F)
...
resights_m <- res_raw %>%
  ...
  merge(inits_m, by = c('AnimalID'), all.y = T) %>% #take only males
  transform(year = year(date)) %>%
  transform(years_old = year - t_0) %>%
  filter(year < 2019) %>%
  transform(SiteName = trimws(SiteName, which = c("both", "left", "right")))  %>%
...
ch_m <- resights_m %>%
  ...
  distinct(AnimalID, year, Region, reg_0, t_0, Sex,
           years_old, age_cat, pup_status_yr, pup_cat, obs, Mass_std) %>%
  dplyr::select(AnimalID, year, Sex, reg_0, t_0, obs, Mass_std) %>%
  distinct() %>% #check why there are duplicates
...
# write.csv(ch_fem,
#           file = paste(here::here('SSL_CJS', 'Data', 'ProcData'), '/ch_fem.csv', sep = ''),
#           row.names = F)
# write.csv(num_resight,
#           file = paste(here::here('SSL_CJS', 'Data', 'ProcData'), '/num_resight.csv', sep = ''),
#           row.names = F)
# write.csv(ch_m,
#           file = paste(here::here('SSL_CJS', 'Data', 'ProcData'), '/ch_m.csv', sep = ''),
#           row.names = F)
# 
# ## Eastern
...
#           file = paste(here::here('SSL_CJS', 'Data', 'ProcData'), '/ch_fem_EAST.csv', sep = ''),
#           row.names = F)
# write.csv(num_resight %>% filter(AnimalID %in% ids$AnimalID),
#           file = paste(here::here('SSL_CJS', 'Data', 'ProcData'), '/num_resight_EAST.csv', sep = ''),
#           row.names = F)
# write.csv(ch_m %>% filter(AnimalID %in% ids_m$AnimalID),
#           file = paste(here::here('SSL_CJS', 'Data', 'ProcData'), '/ch_m_EAST.csv', sep = ''),
#           row.names = F)
# 
# ## Western (WALEU + CALEU)
...
#           file = paste(here::here('SSL_CJS', 'Data', 'ProcData'), '/ch_fem_WEST.csv', sep = ''),
#           row.names = F)
# write.csv(num_resight %>% filter(AnimalID %in% ids$AnimalID) %>%
#             dplyr::select(-c(`2000`:`2010`)),
#           file = paste(here::here('SSL_CJS', 'Data', 'ProcData'), '/num_resight_WEST.csv', sep = ''),
#           row.names = F)
# write.csv(ch_m %>% filter(AnimalID %in% ids_m$AnimalID) %>%
#             dplyr::select(-c(`2000`:`2010`)),
#           file = paste(here::here('SSL_CJS', 'Data', 'ProcData'), '/ch_m_WEST.csv', sep = ''),
#           row.names = F)

```

```{r manuscript inline bits}
...
write.csv(brands_yr_reg, file = here::here('SSL_CJS', 'Data', 'ProcData', 'brands.csv'),
          row.names = F)
