...
# phiNB surv of NB breeders 5+ years old 
...
n.iter = 500; n.chains = 3; n.burnin = 100; nthin = 3; nAdapt = 20
...
##11/1 replaced chla for wind on udrive then had merge conflicts
...
```{r null waleu smooth age mvnorm}
...
# traceplot(out$samples[,grepl('psi', noZ)])
traceplot(out$samples[,'int.phiPM'])
...
saveRDS(out, here::here('results', 'out_null_west_car.rds'))
...
for (i in 1:n_fem) { #females
        for (t in 1:(n_occasions - 1)) {
            logit(phiP[i,t]) <- mu.phiP
            logit(phi1[i,t]) <- mu.phi1
            logit(phi2[i,t]) <- mu.phi2
            logit(phi3[i,t]) <- mu.phi3
            logit(phi4[i,t]) <- mu.phi4
            logit(phi5[i,t]) <- mu.phi5
            logit(phiB[i,t]) <- mu.A
            logit(phiNB[i,t]) <- mu.A
            logit(delB[i,t]) <- b.delB[resights[i,t]]
            logit(p1[i,t]) <- mean.p1 #+ b.eff[eff[t]]
  ...
  for (i in 1:n_m) { #males
        for (t in 1:(n_occasions - 1)) {
            logit(phiPM[i,t]) <- mu.phiPM
            logit(phi1M[i,t]) <- mu.phi1M
            logit(phi2M[i,t]) <- mu.phi2M
            logit(phi3M[i,t]) <- mu.phi3M
            logit(phi4M[i,t]) <- mu.phi4M
            logit(phi5M[i,t]) <- mu.phi5M
            logit(phiBM[i,t]) <- mu.AM
            logit(p1M[i,t]) <- mean.p1M #+ b.eff[eff[t]]
            logit(p2M[i,t]) <- mean.p2M #+ b.eff[eff[t]]
            logit(p3M[i,t]) <- mean.p3M #+ b.eff[eff[t]]
  ...
  # sigma.phiM ~ dexp(1)
    ...
    mu.phiP <- log(int.phiP/(1 - int.phiP))
    mu.phi1 <- log(int.phi1/(1 - int.phi1))
    mu.phi2 <- log(int.phi2/(1 - int.phi2))
    mu.phi3 <- log(int.phi3/(1 - int.phi3))
    mu.phi4 <- log(int.phi4/(1 - int.phi4))
    mu.phi5 <- log(int.phi5/(1 - int.phi5))
    int.phiP ~ dunif(0,1)
    int.phi1 ~ dunif(0,1)
    int.phi2 ~ dunif(0,1)
    int.phi3 ~ dunif(0,1)
    ...
    mu.phiPM <- log(int.phiPM/(1 - int.phiPM))
    mu.phi1M <- log(int.phi1M/(1 - int.phi1M))
    mu.phi2M <- log(int.phi2M/(1 - int.phi2M))
    mu.phi3M <- log(int.phi3M/(1 - int.phi3M))
    mu.phi4M <- log(int.phi4M/(1 - int.phi4M))
    mu.phi5M <- log(int.phi5M/(1 - int.phi5M))
    int.phiPM ~ dunif(0,1)
    int.phi1M ~ dunif(0,1)
    int.phi2M ~ dunif(0,1)
    int.phi3M ~ dunif(0,1)
